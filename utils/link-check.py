...
#
# The first four are missing files rather than bad links: three patch
...
#
# The fifth is different, and worth watching: bug.txt links
# doc/Section_tools.html#implicit, and the manual does define that anchor
# -- Section_tools.txt has "implicit_grid tool :h4,link(implicit)".  The
# copy of the manual committed here under doc/ predates it.  The link is
# right and the copy is stale, so this entry should disappear on its own
# the next time doc/ is refreshed from the SPARTA repository.  If it does
# not, the refresh did not happen.
KNOWN_BROKEN = {
    'bug.html': {'patches/files.11Jun16', 'patches/files.24May16',
                 'patches/files.4Apr17',
                 'doc/Section_tools.html#implicit'},
